 ...
 *                    && npm run build:website -w @supa-agent/website
 *                    && npm run zip -w @supa-agent/ext
 *
 * 1. cleanup
...
tasks.push(
	{
		label: '@supa-agent/website',
		command: 'npm run build:website',
		cwd: join(rootDir, 'packages/website'),
	},
	{ label: '@supa-agent/ext', command: 'npm run zip', cwd: join(rootDir, 'packages/extension') }
)
