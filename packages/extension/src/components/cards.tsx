...
function RetryCard({ event }: { event: RetryEvent }) {
	return (
		<div className="rounded-lg border border-bg-primary/30 bg-primary/10 p-2.5">
			<div className="flex items-start gap-1.5">
				<RefreshCw className="size-3 text-primary shrink-0 mt-0.5" />
...
export function ActivityCard({ activity }: { activity: AgentActivity }) {
	const getActivityInfo = () => {
		switch (activity.type) {
			...
			case 'executing':
				return { text: `Executing ${activity.tool}...`, color: 'text-primary' }
			case 'executed':
