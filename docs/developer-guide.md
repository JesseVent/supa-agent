...
1. **Prerequisites**
    ...
    - An editor that supports `ts/biome`
    - Make sure `biome` and `commitlint` work well. Un-linted code won't pass the CI.
...
- **SupaAgent** (`packages/supa-agent/`) - Main entry with built-in UI Panel (npm: `supa-agent`)
- **MCP** (`packages/mcp/`) - MCP server for browser control via SupaAgent extension (npm: `@supa-agent/mcp`)
- **Core** (`packages/core/`) - Core agent logic without UI (npm: `@supa-agent/core`)
...
- **Ollama example** (tested on 0.15 + qwen3:14b, RTX3090 24GB):
    ...
    > @see https://github.com/JesseVent/supa-agent/blob/main/docs/features/models.md#ollama for configuration
...
- Start and serve a local `iife` script
    ...
    ```bash
    npm run dev:demo # Serving IIFE with auto rebuild at http://localhost:5174/supa-agent.demo.js
    ```

- Add a new bookmark

    ```javascript
    javascript:(function(){var s=document.createElement('script');s.src=`http://localhost:5174/supa-agent.demo.js?t=${Math.random()}`;s.onload=()=>console.log(%27SupaAgent ready!%27);document.head.appendChild(s);})();
    ```

- Click the bookmark on any page to load SupaAgent
