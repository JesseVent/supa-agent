...
if (autoInit) {
	setTimeout(() => {
		...
		if (currentScriptURL) {
			...
			console.log('SupaAgent: no current script detected, using default demo config')
			config = {
		...
		console.log('SupaAgent initialized with config:', window.pageAgent.config)
	})
}
